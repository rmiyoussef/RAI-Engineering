...
### R9 — Model Neutrality
RAI orchestrates the engineering workflow; it never hard-codes a model. Agents run on the host tool's default model. Optionally, `.brain/config.yaml` may route agent roles to model tiers (`fast`/`balanced`/`deep`/`architect`). No config ⇒ host default everywhere, zero disruption.

**Violation:** Assuming or requiring a specific vendor model.
...
Every agent-to-agent message must follow the Message Protocol defined in `reference/message-protocol.md`. Messages missing required fields are rejected.
...
### R17 — Always Read Context First
Before any planning or execution, read `ARCHITECTURE.md`, `INSTRUCTIONS.md`, `INDEX.md`, `state/current.yaml`, and the relevant `context/` + `knowledge/` files. If context is missing for the affected area, call ARCHITECT to create it from project analysis.

**Violation:** The BRAIN must justify why context was not consulted.

### R18 — Always Read Memory Before Writing
Before making any decision or writing any code, check existing memory:
- Check `INDEX.md` + `state/` for relevant entries and active work
- Check `memory/decisions/` for past decisions about this area
- Check `memory/lessons/` for known pitfalls
- Check `knowledge/` for the current system map

**Violation:** The agent's output is rejected until memory is consulted.

### R19 — Update Context & Knowledge When Architecture Changes
If a task introduces a new pattern, command, middleware, convention, or technology, ARCHITECT must update `context/` + `knowledge/` (filed by purpose, tagged with `domains:` metadata) to reflect the change. They must always represent the current state of the project.

**Violation:** MEMORY SCRIBE flags the session as incomplete.

### R20 — Never Push Connection Info to Git
The `context/connections/` directory contains database schema and connection information. It must never be committed to Git. The `.gitignore` must include `context/connections/`. The DATABASE agent must verify this before writing any connection file.
...
## Purpose-Organization Rules (R36-R40)

### R36 — Area Tags Required
Every task must tag its affected technical areas (backend, frontend, database, security, infra, ...) as `domains:` metadata before work begins. Areas are never directories. If the areas are unknown, the Brain must ask the user before proceeding. Never guess or assume.

### R37 — Purpose-Organized Storage
Plans, rules, skills, knowledge, and memory are filed by purpose (`plans/`, `rules/<purpose>/`, `skills/`, `knowledge/<purpose>/`, `memory/`). Technical-domain folders must never be created inside `.brain`. Each artifact carries `domains:` metadata where relevant.

### R38 — Cross-Area Reference Protocol
When a task spans multiple areas, keep one plan and reference the relevant purpose-organized files using IDs and relative links — never duplicate content across locations. Cross-area references must be explicit, not implicit.

### R39 — Purpose-Scoped Rules
Rules and skills live under purpose categories (e.g., `rules/database/query-optimization.md`, not a domain folder). If multiple frameworks exist in one area, each gets its own file prefix or subdirectory within the purpose category.

### R40 — Brain Structure Check
When starting work, verify the purpose-organized structure (`plans/`, `test-cases/`, `summaries/`, `memory/`, `knowledge/`, `rules/`, `state/`) exists. Never write knowledge without first verifying the target location per `ARCHITECTURE.md`.
