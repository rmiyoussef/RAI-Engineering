...
4. **No model hard-coding.** The brain is model-neutral: agents run on the host tool's default model. Optional per-agent tiers via `.brain/config.yaml` (see R9). Never assume a specific vendor model.
