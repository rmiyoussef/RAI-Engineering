...
```json
{
  ...
  "model": "<host-model, e.g. claude-opus-4>"
}
```
...
| Sender model matches registry entry | Reject on mismatch |
...
5. **Identity consistency.** Every inter-session message validates that the sender's session is registered; the declared model must match the registry entry (any model allowed — R9 model-neutral). Messages from unregistered sessions are rejected.
