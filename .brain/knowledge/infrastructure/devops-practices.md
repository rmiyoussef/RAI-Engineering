...
These rules live in `.brain/rules/<purpose>/` (tagged `domains: [devops]`) and are loaded selectively by sub-task:

| Rule file | Loaded when |
|-----------|-------------|
| `rules/infrastructure/containers.md` | Building Dockerfiles, container security, image optimization |
| `rules/infrastructure/kubernetes.md` | K8s manifests, deployments, network policies, HPA |
| `rules/infrastructure/ci-cd.md` | Setting up or modifying CI/CD pipelines |
| `rules/infrastructure/infra-as-code.md` | Terraform modules, state management, IaC security |
| `rules/infrastructure/cloud-services.md` | Cloud architecture, VPC, IAM, storage, databases |
