...
User request arrives
    |
[1] **TAG DOMAINS AS METADATA** — Note affected technical areas (backend, frontend,
    |   database, security, infra, ...) as `domains:` metadata. Domains never
    |   become directories; knowledge is organized by purpose (ARCHITECTURE.md §2).
    |
[2] BRAIN reads .brain/ARCHITECTURE.md + .brain/INSTRUCTIONS.md  ← Always first
    |
[3] BRAIN reads .brain/state/current.yaml  ← What is currently active?
    |
[4] BRAIN reads .brain/INDEX.md         ← Where does relevant knowledge live?
    |   (if no INDEX.md, project is new)
    |
[5] BRAIN selectively loads relevant context/ files
    |   (if context missing for affected area, call ARCHITECT to create from analysis)
    |
[6] BRAIN loads applicable rules/<purpose>/ files (selected by sub-task)
    |
...
[10] If task involves database:
    ...
    |
[11] If task involves security:
    ├─► BRAIN loads rules/security/ + knowledge/security/
    └─► BRAIN calls SECURITY agent
    |
[12] BRAIN creates session UUID
    |
[13] BRAIN calls ORCHESTRATOR for session init
    ├── Register in .brain/sessions/live/
    ├── Poll inbox for pending inter-session messages
    └── Discover peers in session registry
    |
[14] BRAIN checks if task has independent sub-tasks or spans multiple areas
    |
    ├─► Complex → call ORCHESTRATOR ENGINE (Phase 0b)
    ...
    |
    └─► Single sub-task → route to PLANNER directly
    |
[15] BRAIN routes to PLANNER (or appropriate agent based on task type).
    |   PLANNER creates plan + tasks + initial test cases (INSTRUCTIONS.md §§3-4).
    |   Completion requires implementation + tests + summary (INSTRUCTIONS.md §8).
```

**R17** says: Always read context + INDEX first. If context is missing, ARCHITECT creates it.
...
| Model assumed vendor-specific | Reject with "model neutrality violation (R9)" |
